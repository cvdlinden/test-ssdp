...
const UPnPExplorer = (() => {
    ...
    function renderDevices(devices) {
        ...
        devicesContainer.innerHTML = devices
            .map(d => `
        <div class="list-item" data-device-id="${d.ssdp.LOCATION}">
          ${d.device.friendlyName || 'Unknown Device'}
          <small>
            ${d.device.manufacturer ? d.device.manufacturer[0] : ''}
            ${d.device.modelDescription ? "| " + d.device.modelDescription[0] : ''}
            ${d.device.modelName ? "| " + d.device.modelName[0] : ''}
            ${d.device.modelNumber ? "| " + d.device.modelNumber[0] : ''}
          </small>
          <small>
            ${d.ip.address || 'unknown'} | ${d.ssdp.LOCATION}
          </small>
          <small>
            ${d.device.deviceType[0] || ''}
          </small>
        </div>
      `)
            .join('');
